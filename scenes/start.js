...
const startScreen = {
	[STEPS.FIRST]: async function ({ ctx, user }) {
		const deeplink = ctx.message.text.split(' ').length > 1 ? ctx.message.text.split(' ')[1] : false;
			...
			await fetch(`${process.env.BACKEND_URL}/customers/add`, {
				...
				.then(async (data) => {
					if (data.hash) {
						...
						user.phone = null;
						ctx.reply('Упс, случилась непредвиденная ошибка. Пожалуйста, повторите процедуру с начала 😅');
